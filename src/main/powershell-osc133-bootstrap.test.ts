...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('history.persistence="none"')
    expect(script).toContain('$env:CODEX_HOME = $env:ORCA_CODEX_HOME')
    expect(script).toContain("$arg -eq '--'")
    expect(script).toContain('[void]$codexArgs.Add(\'history.persistence="none"\')')
    expect(script).toContain('& $codexCommand.Source @($codexArgs.ToArray())')
